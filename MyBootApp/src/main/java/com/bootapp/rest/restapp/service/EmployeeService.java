...
public class EmployeeService {
	...
	public List<Employee> getEmployeeByDepartmentID(int did) {
		...
		List<Employee> filteredList = list.stream().filter(e -> e.getDepartment().getId() == did)
	...
	public List<Employee> getEmployeesBySalaryAndCity(double salary, String city) {
		...
		List<Employee> filteredList=list.stream().filter(e->e.getSalary()>salary).filter(e->e.getCity().equals(city)).collect(Collectors.toList());
